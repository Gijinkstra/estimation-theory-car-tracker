...
h       = 0.01;             % step                                 [s]
T_sim   = 5;               % total simulated time                 [s]
...
% ---- Per-step timing ---------------------------------------------------
step_times = zeros(N, 1);    % wall-clock time spent on each iteration [s]

t_loop = tic;

for k = 1:N
    t_iter = tic;             % start per-iteration timer
    ...
    step_times(k) = toc(t_iter);    % record this iteration's solve time
...
fprintf('  per-step max  = %.1f ms\n',  1000*max(step_times));
fprintf('  per-step 99%% = %.1f ms\n',  1000*prctile(step_times, 99));
fprintf('  steps over real-time deadline (%.0f ms): %d / %d  (%.1f%%)\n', ...
        1000*h, sum(step_times > h), N, 100*sum(step_times > h)/N);
...
% --- Figure 4: Per-step solver timing -----------------------------------
figure('Position',[200 80 1300 500], 'Name','MHE: Per-step timing');

t_step = (1:N) * h;     % simulation time corresponding to each iteration

subplot(1,2,1);
semilogy(t_step, 1000*step_times, 'b-', 'LineWidth', 1.0); hold on; grid on;
yline(1000*h, 'r--', sprintf('real-time deadline (%.0f ms)', 1000*h), ...
      'LineWidth', 1.5, 'LabelHorizontalAlignment','left');
xline(N_mhe*h, 'g:', 'window full', 'LabelVerticalAlignment','bottom');
xlabel('simulation time [s]');
ylabel('solve time per step [ms]');
title(sprintf('Per-step solve time  (mean = %.1f ms, max = %.1f ms)', ...
              1000*mean(step_times), 1000*max(step_times)));

subplot(1,2,2);
sorted_ms = sort(1000*step_times);
cdf       = (1:N)' / N;
plot(sorted_ms, 100*cdf, 'b-', 'LineWidth', 1.4); hold on; grid on;
xline(1000*h, 'r--', 'deadline', 'LineWidth', 1.5);
xlabel('solve time per step [ms]');
ylabel('cumulative fraction of steps [%]');
frac_under = 100 * sum(step_times <= h) / N;
title(sprintf('CDF of step times  (%.1f%% under deadline)', frac_under));
