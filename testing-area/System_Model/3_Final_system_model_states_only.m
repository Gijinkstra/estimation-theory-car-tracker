...
% Layout: 4x2 grid. Left column (rows 1-4) is the track overview; the right
% column holds one subplot per state variable (s, v, l, vl) so all four
% dynamics are visible at a glance.
figure('Position',[80 80 1400 900], 'Name','Dynamics only (no beacons)');

% --- Track & trajectory (left column, all 4 rows) ---
subplot(4,2,[1 3 5 7]); hold on; axis equal; grid on;
...
legend({'centre line','inner wall','outer wall','vehicle path'}, ...
       'Location','southoutside','Orientation','horizontal');

% --- State 1: s (arc-length position) ---
subplot(4,2,2); hold on; grid on;
plot(t, X(1,:), 'b-', 'LineWidth',1.0);
xlabel('time [s]'); ylabel('s [m]');
title('s  - arc-length position (sawtooth as it laps the loop)');

% --- State 2: v (longitudinal speed) ---
subplot(4,2,4); hold on; grid on;
plot(t, X(2,:), 'b-', 'LineWidth',1.0);
yline(x0(2), 'k--', 'LineWidth',0.6);    % reference: nominal cruise speed v_0
xlabel('time [s]'); ylabel('v [m/s]');
title('v  - longitudinal speed (random walk from v_0)');

% --- State 3: l (lateral position) ---
subplot(4,2,6); hold on; grid on;
plot(t, X(3,:), 'r-', 'LineWidth',1.0);
yline( l_max, 'k--', 'LineWidth',0.6);
yline(-l_max, 'k--', 'LineWidth',0.6);
yline(0,      'k:',  'LineWidth',0.5);
xlabel('time [s]'); ylabel('l [m]');
title('l  - lateral position (clamped at +-l_{max})');

% --- State 4: vl (lateral speed) ---
subplot(4,2,8); hold on; grid on;
plot(t, X(4,:), 'r-', 'LineWidth',1.0);
yline(0, 'k:', 'LineWidth',0.5);
xlabel('time [s]'); ylabel('v_l [m/s]');
title('v_l - lateral speed (random walk driven by Beta noise w_l)');
