...
% ---- Process noise ------------------------------------------------------
sigma_v = 0.25;             % Std of longitudinal speed noise w_v [m/s] (ASSUMED)

% Lateral (Beta) noise design:
%     X ~ Beta( a(l), b(l) ) ,   noise = beta_scale * ( X - a/(a+b) )
% so E[noise] = 0 by construction. The SHAPE depends on l:
%   - concentration (a+b) drops as |l| -> l_max  =>  heavier-tailed / noisier near walls
%   - skew is pushed so the mode of X is on the 'wall' side when l is off-centre,
%     meaning the zero-mean deviation (X - mean) tends to produce more mass of
%     small negative pushes and rarer large positive pushes (or vice-versa).
...
for k = 1:N
    ...
    % Beta-distributed lateral noise whose shape depends on l
    conc = max(beta_a0 - beta_k_conc*(abs(l)/l_max), 1.5);
    skew = -beta_k_skew * (l/l_max);         % l>0 -> skew<0 -> distribution pushed left
    ...
    % Enforce |l| <= l_max with a simple velocity-reflecting wall
    if l_new >  l_max, l_new =  l_max; vl_new = -0.2*vl_new; end
    if l_new < -l_max, l_new = -l_max; vl_new = -0.2*vl_new; end

    X(:,k+1) = [s_new; v_new; l_new; vl_new];

    % ---- world-frame position for measurements -------------------------
    [xw, yw] = sl_to_xy(s_new, l_new, geom);
    XY(:,k+1) = [xw; yw];

    % ---- nonlinear range measurements from the 3 beacons --------------
    for i = 1:Nb
        Y(i,k) = sqrt( (xw - beacons(i,1))^2 + (yw - beacons(i,2))^2 ) ...
                 + sigma_r*randn;
    end

    % ---- recover s from beacons ---------------------------------------
