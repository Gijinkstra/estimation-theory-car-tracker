...
% Direction of loop traversal:
%   s_dir = +1 -> car goes CCW around the loop (bottom-straight first, rightward)
%   s_dir = -1 -> car goes CW  around the loop (big arc first, leftward initially)
% s = 0 stays at the same physical point (P1_bot) in either case; only the
% direction of forward motion is flipped. Implemented as a transform of s
% when handing it to the geometry functions, so the state-space equations
% themselves are unchanged.
s_dir = -1;                 % traverse loop in reversed (CW) direction (NEW)
s_geom = @(s) mod(s_dir*s, L_total);     % external s -> geometric (CCW) s
s_user = @(s) mod(s_dir*s, L_total);     % geometric s -> external s (same map)
...
% long runs v will wander (by design - this is what the user asked for).
sigma_v  = 0.25;            % Std of zero-mean longitudinal speed noise [m/s] (ASSUMED)
...
beta_scale  = 1.5;          % m/s scale of the lateral noise        (ASSUMED)
...
[XY(1,1), XY(2,1)] = sl_to_xy(s_geom(X(1,1)), X(3,1), geom);
...
for k = 1:N
    ...
    % is what the beacons actually see. The s_geom() wrapper applies the
    % loop-direction flag s_dir so positive v means "forward" in whichever
    % direction the user has chosen.
    [xw, yw] = sl_to_xy(s_geom(s_new), l_new, geom);
    XY(:,k+1) = [xw; yw];
    ...
    S_est(k) = s_user(s_hat);    % undo the direction transform so S_est is in
                                 % the same convention as the simulated s
    L_est(k) = l_hat;
end
...
playback_dt = 0.15;          % wall-clock delay between frames          [s]
